...
function exportNotebookMarkdown(){
  ...
  const lines=[
    ...
    '## Risiko / asersi',state.risk||'_Belum diisi_','',
    '## Kontrol yang diharapkan',state.expectedControl||'_Belum diisi_','',
    '## Bukti dan keterbatasan',state.evidenceNote||'_Belum diisi_','',
    '## Petunjuk transaksi',...(selectedClues.length?selectedClues.map(t=>`- ${t.id}: ${t.sender} → ${t.beneficiary} — ${t.amount}`):['_Belum ada petunjuk dipilih_']),'',
